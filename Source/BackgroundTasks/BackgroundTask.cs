...
[Guid("715EBBEE-2014-DEAD-BEEF-5B201C6A8056")] // {87654321-1234-1234-1234-1234567890AE} is GUID to register this task with BackgroundTaskBuilder. Generate a random GUID before implementing.
...
public class BackgroundTask : IBackgroundTask, IDisposable
{
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disposed)
        {
            if (disposing)
            {
                // Stop the server when the background task is disposed (via ManualResetEvent).
                //App.SignalExit();
            }
            disposed = true;
        }
    ...
    {
        // Unregister the task when the task is destroyed (via ManualResetEvent)
        //App.SignalExit();
    }
}
