...
public static partial class ScrollViewerExtensions
{
    ...
    static bool ChangeVerticalScrollBarMarginProperty(FrameworkElement sender)
    {
        if (sender == null)
        {
            return false;
    ...
    static void ChangeVerticalScrollBarMarginProperty(object sender, RoutedEventArgs routedEventArgs)
    {
        if (sender is FrameworkElement baseElement)
        {
    ...
    public static readonly DependencyProperty ScrollTargetProperty = DependencyProperty.RegisterAttached(
        ...
        (obj, args) =>
        {
            ...
            if (!(obj is ListViewBase lvb))
                throw new InvalidOperationException($"The '{nameof(ScrollTargetProperty)}' property should only be used with 'Microsoft.UI.Xaml.Controls.ListViewBase'.");
