...
public sealed partial class PlotPage : Page
{
    ...
    List<string> _sizes = new List<string>() { "4", "6", "8", "10", "12", "16", "20", "24", "30" };
    List<string> _delays = new List<string>() { "2", "4", "8", "10", "14", "20", "30", "40", "80" };
    #endregion
    ...
    public PlotPage()
    {
        ...
        cmbSizes.SelectedItem = _sizes[3];
    ...
    {
        cvsPlot.Width = App.Profile!.WindowWidth - 80;
        cvsPlot.Height = App.Profile!.WindowHeight - 120;
    ...
    void CircleOnPointerEntered(object sender, PointerRoutedEventArgs e)
    {
        ...
        GeneralTransform transform = circle.TransformToVisual(this); // or PlotCanvas
        Point position = transform.TransformPoint(new Point(circle.Width / 2, circle.Height / 2)); // Center of the circle
        Debug.WriteLine($"[INFO] Position data is X={position.X}, Y={position.Y}");
        ...
        ttPlot.Placement = PlacementMode.Mouse;
        ttPlot.HorizontalOffset = _circleRadius;  // X offset from mouse
        ttPlot.VerticalOffset = _circleRadius;    // Y offset from mouse
